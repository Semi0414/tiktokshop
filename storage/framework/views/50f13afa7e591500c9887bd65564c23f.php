<?php echo $__env->make('admin::seller.partials.sse-modal-styles', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    ...
    >
            ...
            <div class="sse-modal__header-main">
                ...
                <div class="min-w-0">
                    <h2 id="warehouse-add-modal-title" class="sse-modal__title">
                        <?php echo app('translator')->get('admin::app.seller-panel.product-warehouse.bulk-add-modal-title'); ?>
                    </h2>
                    <p id="warehouse-add-modal-hint" class="sse-modal__subtitle">
                        <?php echo app('translator')->get('admin::app.seller-panel.product-warehouse.bulk-add-modal-hint'); ?>
                    </p>
                </div>
            </div>
            <button type="button" id="warehouse-add-modal-close" class="sse-modal__close" data-warehouse-modal-dismiss aria-label="<?php echo app('translator')->get('admin::app.acl.cancel'); ?>">
                <span aria-hidden="true">&times;</span>
            </button>
        </header>

        <div class="sse-modal__body">
            <form id="warehouse-add-modal-form" method="post" action="<?php echo e(route('admin.seller.product-warehouse.attach')); ?>" class="flex flex-col gap-4">
                <?php echo csrf_field(); ?>
                <div class="sse-field sse-field--delay-1">
                    <label class="sse-field__label" for="warehouse-add-modal-commission">
                        <?php echo app('translator')->get('admin::app.seller-panel.product-warehouse.bulk-add-seller-profit-label'); ?>
                    </label>
                    <input
        ...
        <footer class="sse-modal__footer">
            <button type="button" class="sse-modal__btn sse-modal__btn--ghost" data-warehouse-modal-dismiss>
                <?php echo app('translator')->get('admin::app.acl.cancel'); ?>
            </button>
            <button type="submit" form="warehouse-add-modal-form" class="sse-modal__btn sse-modal__btn--primary">
                <?php echo app('translator')->get('admin::app.seller-panel.product-warehouse.bulk-add-confirm'); ?>
            </button>
        </footer>
    </div>
</div>
...
(function () {
    var existingProductIds = new Set(
        (<?php echo json_encode($sellerExistingProductIds ?? [], 15, 512) ?>).map(function (id) {
    ...
    function warehouseOpenAddModalWithIds(productIds) {
        ...
        if (!normalizedIds.length) {
            warehouseNotify('warning', <?php echo json_encode(__('admin::app.components.datagrid.index.no-records-selected'), 15, 512) ?>);
...
<?php /**PATH /Users/hhtraders/tiktokshop/packages/Webkul/Admin/src/resources/views/seller/product-warehouse/partials/add-modal.blade.php ENDPATH**/ ?>