...
namespace Webkul\SuperAdmin\Validations;
...
class ProductCategoryUniqueSlug implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        if (in_array($value, $this->reservedSlugs)) {
            $fail('superadmin::app.validations.slug-reserved')->translate();

            return;
        }

        if (! $this->isSlugUnique($value)) {
            $fail('superadmin::app.validations.slug-being-used')->translate();
        }
    }
