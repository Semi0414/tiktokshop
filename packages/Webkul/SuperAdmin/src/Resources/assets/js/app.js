...
/**
 * Mount Vue once per full page load (classic full navigation).
 *
 * Defer so Blade @stack('scripts') module tags (v-superadmin-dropdown, etc.)
 * run before mount — same order as Admin: Vite module first, then body modules.
 */
function mountBagistoVue() {
    if (window.__bagistoVueMounted) {
        return;
    }
