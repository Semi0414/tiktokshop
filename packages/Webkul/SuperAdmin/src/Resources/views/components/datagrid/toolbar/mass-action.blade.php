            ...
            <div class="flex w-full items-center gap-x-1">
                <x-superadmin::dropdown>
                    <x-slot:toggle>
                        ...
                        >
                            <span>
                                @lang('superadmin::app.components.datagrid.toolbar.mass-actions.select-action')
                            </span>
                    ...
                    <x-slot:menu class="!p-0 shadow-[0_5px_20px_rgba(0,0,0,0.15)] dark:border-gray-800 max-h-[min(70vh,24rem)] overflow-y-auto">
                ...
                <div class="ltr:pl-2.5 rtl:pr-2.5">
                    <p class="text-sm font-light text-gray-800 dark:text-white">
                        @{{ "@lang('superadmin::app.components.datagrid.toolbar.length-of')".replace(':length', applied.massActions.indices.length) }}

                        @{{ "@lang('superadmin::app.components.datagrid.toolbar.selected')".replace(':total', available.meta.total) }}
                    </p>
                </div>
            </div>
        </slot>
            ...
            methods: {
                ...
                validateMassAction() {
                    if (! this.applied.massActions.indices.length) {
                        this.$emitter.emit('add-flash', { type: 'warning', message: "@lang('superadmin::app.components.datagrid.index.no-records-selected')" });

                        return false;
                    }

                    if (! this.applied.massActions.meta.action) {
                        this.$emitter.emit('add-flash', { type: 'warning', message: "@lang('superadmin::app.components.datagrid.index.must-select-a-mass-action')" });
                    ...
                    if (
                        ...
                        this.$emitter.emit('add-flash', { type: 'warning', message: "@lang('superadmin::app.components.datagrid.index.must-select-a-mass-action-option')" });
