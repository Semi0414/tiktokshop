<x-superadmin::layouts>
    <!-- Title of the page. -->
    <x-slot:title>
        @lang('superadmin::app.configuration.index.title')
    </x-slot>
        ...
        <v-configuration-search>
            <div class="relative flex w-[525px] max-w-[525px] items-center max-lg:w-[400px] ltr:ml-2.5 rtl:mr-2.5">
                ...
                <input
                    ...
                    placeholder="@lang('superadmin::app.configuration.index.search')"
                >
            </div>
        </v-configuration-search>
    </div>
                ...
                <div class="box-shadow max-1580:grid-cols-3 mt-2 grid grid-cols-4 flex-wrap justify-between gap-12 rounded bg-white p-4 dark:bg-gray-900 max-xl:grid-cols-2 max-sm:grid-cols-1">
                    ...
                    @foreach ($item->getChildren() as $key => $child)
                        <a
                            ...
                            href="{{ route('superadmin.configuration.index', ($item->getKey() . '/' . $key)) }}"
                        >
    ...
    @pushOnce('scripts')
                ...
                >
                    ...
                    <template v-else>
                        <div class="grid max-h-[400px] overflow-y-auto">
                            ...
                            >
                                @lang('superadmin::app.configuration.index.no-result-found')
                            </div>
                        </div>
                    </template>
                </div>
        ...
        <script type="module">
            window.app.component('v-configuration-search', {
                ...
                methods: {
                    search() {
                        ...
                        this.$axios.get("{{ route('superadmin.configuration.search') }}", {
