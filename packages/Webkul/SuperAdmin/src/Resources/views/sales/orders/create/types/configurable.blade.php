...
@pushOnce('scripts')
        ...
        <div class="w-[455px] max-w-full p-4">
            ...
            <template v-for='(attribute, index) in childAttributes'>
                <x-superadmin::form.control-group>
                    ...
                    <x-superadmin::form.control-group.error ::name="'super_attribute[' + attribute.id + ']'" />
                </x-superadmin::form.control-group>
            </template>
        </div>
    </script>
    ...
    <script type="module">
        window.app.component('v-product-configurable-options', {
            ...
            methods: {
                getAttibuteOptions() {
                    ...
                    this.$axios.get("{{ route('superadmin.catalog.products.configurable.options', ':replace') }}".replace(':replace', this.productOptions.product.id))
                        .then(response => {
                ...
                fillSelect(attribute) {
                    ...
                    attribute.options = [{
                        'id': '',
                        'label': "@lang('superadmin::app.sales.orders.create.types.configurable.select-options')",
                        'products': []
                    }];
