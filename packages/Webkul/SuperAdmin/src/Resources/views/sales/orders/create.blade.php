<x-superadmin::layouts>
    <!-- Title of the page -->
    <x-slot:title>
        @lang('superadmin::app.sales.orders.create.title', ['name' => e($cart->customer->name)])
    </x-slot>
    ...
    <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
        ...
        >
            @lang('superadmin::app.sales.orders.create.back-btn')
        </a>
    </div>
    ...
    <v-create-order>
        ...
        <x-superadmin::shimmer.sales.orders.create />
    </v-create-order>
    ...
    @pushOnce('scripts')
        ...
        >
            <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
                ...
                >
                    ...
                    >
                        <form
                            ...
                            ref="addToCartForm"
                        >
                            <x-superadmin::drawer ref="productConfigurationDrawer">
                                <!-- Drawer Header -->
                                <x-slot:header>
                                    <div class="flex items-center justify-between">
                                        <p class="text-xl font-medium dark:text-white">
                                            @lang('superadmin::app.sales.orders.create.configuration')
                                        </p>

                                        <button class="primary-button ltr:mr-11 rtl:ml-11">
                                            @lang('superadmin::app.sales.orders.create.add-to-cart')
                                        </button>
                                    </div>
                                </x-slot>
                ...
                <div class="flex w-[360px] max-w-full flex-col gap-2 max-sm:w-full">
                    ...
                    @include('superadmin::sales.orders.create.wishlist-items')

                    <!-- Compare Items Component -->
                    @include('superadmin::sales.orders.create.compare-items')

                    <!-- Recent Order Items Component -->
                    @include('superadmin::sales.orders.create.recent-order-items')
                </div>
        ...
        <script type="module">
            window.app.component('v-create-order', {
                ...
                methods: {
                    ...
                    getCart() {
                        this.$axios.get("{{ route('superadmin.sales.cart.index', $cart->id) }}")
                            .then(response => {
                                this.cart = response.data.data;
                            })
                            .catch(error => {
                                window.location.href = "{{ route('superadmin.sales.orders.customers.index') }}";
                            });
                    },
                    ...
                    addToCart(params) {
                        ...
                        this.$axios.post("{{ route('superadmin.sales.cart.items.store', $cart->id) }}", formData)
                            .then(response => {
