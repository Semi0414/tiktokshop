<x-superadmin::layouts>
    <x-slot:title>
        @lang('superadmin::app.sales.orders.view.title', ['order_id' => $order->increment_id])
    </x-slot>
            ...
            <div class="flex items-center gap-2.5">
                ...
                @php
                    $headerOrderStatusKey = ($order->seller_approval_status ?? null) === 'pending'
                        ...
                        : $order->status;
                @endphp
                <span class="label-{{ $headerOrderStatusKey }} text-sm mx-1.5">
                    @lang("superadmin::app.sales.orders.view.$headerOrderStatusKey")
                </span>
            </div>
    ...
    <div class="mt-5 flex-wrap items-center justify-between gap-x-1 gap-y-2">
        <div class="flex gap-1.5">
            ...
            @if (
                ...
                <a
                    href="{{ route('superadmin.sales.orders.reorder', $order->id) }}"
                    class="transparent-button px-1 py-1.5 hover:bg-gray-200 dark:text-white dark:hover:bg-gray-800"
                >
                    <span class="icon-cart text-2xl"></span>

                    @lang('superadmin::app.sales.orders.view.reorder')
                </a>
            @endif
            ...
            @if (
                ...
                @include('superadmin::sales.invoices.create')
            @endif
            ...
            @if (
                ...
                @include('superadmin::sales.shipments.create')
            @endif
            ...
            @if (
                ...
                @include('superadmin::sales.refunds.create')
            @endif
            ...
            @if (
               ...
               <form
                    method="POST"
                    ref="cancelOrderForm"
                    action="{{ route('superadmin.sales.orders.cancel', $order->id) }}"
                >
                    @csrf
                </form>
                ...
                <div
                    ...
                    @click="$emitter.emit('open-confirm-modal', {
                        message: '@lang('superadmin::app.sales.orders.view.cancel-msg')',
                        agree: () => {
                ...
                >
                    ...
                    <a href="javascript:void(0);">
                        @lang('superadmin::app.sales.orders.view.cancel')
                    </a>
                </div>
            @endif
        ...
        @if (isset($sellers))
            <div class="mt-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
                <p class="mb-3 text-base font-semibold text-gray-800 dark:text-white">
                    @lang('superadmin::app.sales.orders.view.seller-marketplace')
                </p>

                <form
                    method="POST"
                    action="{{ route('superadmin.sales.orders.assign_seller', $order->id) }}"
                ...
                >
                    ...
                    <div class="min-w-[220px]">
                        <label class="mb-1 block text-sm text-gray-600 dark:text-gray-300">
                            @lang('superadmin::app.sales.orders.view.assign-seller')
                        </label>
                        ...
                        >
                            <option value="">
                                @lang('superadmin::app.sales.orders.view.no-seller')
                            </option>

                            @foreach ($sellers as $sellerOption)
                                <option
                                    value="{{ $sellerOption->id }}"
                                    @selected((int) $order->seller_id === (int) $sellerOption->id)
                                >
                                    {{ $sellerOption->name }} ({{ $sellerOption->email }})
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    >
                        @lang('superadmin::app.sales.orders.view.save-seller')
                    </button>
                </form>

                @if ($order->seller_id)
                    <p class="mt-3 text-sm text-gray-600 dark:text-gray-300">
                        @lang('superadmin::app.sales.orders.view.seller-approval-status'):
                        <span class="font-semibold text-gray-800 dark:text-white">
                            {{ $order->seller_approval_status ?? '—' }}
                        </span>
                    </p>
                @endif
            </div>
        ...
        <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
            ...
            <div class="flex flex-1 flex-col gap-2 max-xl:flex-auto">
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <div class="flex justify-between p-4">
                        ...
                        <p class="text-base font-semibold text-gray-800 dark:text-white">
                            @lang('superadmin::app.sales.orders.view.grand-total', ['grand_total' => core()->formatBasePrice($order->base_grand_total)])
                        </p>
                    </div>
                                    ...
                                    <div class="grid place-content-start gap-1.5">
                                        ...
                                        <div class="flex flex-col place-items-start gap-1.5">
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.amount-per-unit', [
                                            ...
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.sku', ['sku' => $item->getTypeInstance()->getOrderedItem($item)->sku ])
                                            </p>

                                            <p class="text-gray-600 dark:text-gray-300">
                                                {{ $item->qty_ordered ? trans('superadmin::app.sales.orders.view.item-ordered', ['qty_ordered' => $item->qty_ordered]) : '' }}

                                                {{ $item->qty_invoiced ? trans('superadmin::app.sales.orders.view.item-invoice', ['qty_invoiced' => $item->qty_invoiced]) : '' }}

                                                {{ $item->qty_shipped ? trans('superadmin::app.sales.orders.view.item-shipped', ['qty_shipped' => $item->qty_shipped]) : '' }}

                                                {{ $item->qty_refunded ? trans('superadmin::app.sales.orders.view.item-refunded', ['qty_refunded' => $item->qty_refunded]) : '' }}

                                                {{ $item->qty_canceled ? trans('superadmin::app.sales.orders.view.item-canceled', ['qty_canceled' => $item->qty_canceled]) : '' }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="grid place-content-start gap-1">
                                    ...
                                    <div class="flex flex-col place-items-start items-end gap-1.5">
                                        @if (core()->getConfigData('sales.taxes.sales.display_prices') == 'including_tax')
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.price', ['price' => core()->formatBasePrice($item->base_price_incl_tax)])
                                            </p>
                                        @elseif (core()->getConfigData('sales.taxes.sales.display_prices') == 'both')
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.price-excl-tax', ['price' => core()->formatBasePrice($item->base_price)])
                                            </p>

                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.price-incl-tax', ['price' => core()->formatBasePrice($item->base_price_incl_tax)])
                                            </p>
                                        @else
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.price', ['price' => core()->formatBasePrice($item->base_price)])
                                            </p>
                                        @endif

                                        <p class="text-gray-600 dark:text-gray-300">
                                            @lang('superadmin::app.sales.orders.view.tax', [
                                        ...
                                        @if ($order->base_discount_amount > 0)
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.discount', ['discount' => core()->formatBasePrice($item->base_discount_amount)])
                                            </p>
                                        @endif

                                        @if (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'including_tax')
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.sub-total', ['sub_total' => core()->formatBasePrice($item->base_total_incl_tax)])
                                            </p>
                                        @elseif (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'both')
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.sub-total-excl-tax', ['sub_total' => core()->formatBasePrice($item->base_total)])
                                            </p>

                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.sub-total-incl-tax', ['sub_total' => core()->formatBasePrice($item->base_total_incl_tax)])
                                            </p>
                                        @else
                                            <p class="text-gray-600 dark:text-gray-300">
                                                @lang('superadmin::app.sales.orders.view.sub-total', ['sub_total' => core()->formatBasePrice($item->base_total)])
                                            </p>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div class="mt-4 flex flex-auto justify-end p-4">
                        <div class="grid max-w-max gap-2 text-sm">
                            ...
                            @if (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'including_tax')
                                <div class="flex w-full justify-between gap-x-5">
                                    <p class="font-semibold !leading-5 text-gray-600 dark:text-gray-300">
                                        @lang('superadmin::app.sales.orders.view.summary-sub-total-incl-tax')
                                    </p>
                            ...
                            @elseif (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'both')
                                <div class="flex w-full justify-between gap-x-5">
                                    <p class="font-semibold !leading-5 text-gray-600 dark:text-gray-300">
                                        @lang('superadmin::app.sales.orders.view.summary-sub-total-excl-tax')
                                    </p>
                            ...
                            @else
                                <div class="flex w-full justify-between gap-x-5">
                                    <p class="font-semibold !leading-5 text-gray-600 dark:text-gray-300">
                                        @lang('superadmin::app.sales.orders.view.summary-sub-total')
                                    </p>
                            ...
                            @if ($haveStockableItems = $order->haveStockableItems())
                                @if (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'including_tax')
                                    <div class="flex w-full justify-between gap-x-5">
                                        <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                            @lang('superadmin::app.sales.orders.view.shipping-and-handling-incl-tax')
                                        </p>
                                ...
                                @elseif (core()->getConfigData('sales.taxes.sales.display_shipping_amount') == 'both')
                                    <div class="flex w-full justify-between gap-x-5">
                                        <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                            @lang('superadmin::app.sales.orders.view.shipping-and-handling-excl-tax')
                                        </p>
                                ...
                                @else
                                    <div class="flex w-full justify-between gap-x-5">
                                        <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                            @lang('superadmin::app.sales.orders.view.shipping-and-handling')
                                        </p>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.summary-tax')
                                </p>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.summary-discount')
                                </p>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                <p class="text-base font-semibold !leading-5 text-gray-800 dark:text-white">
                                    @lang('superadmin::app.sales.orders.view.summary-grand-total')
                                </p>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.total-paid')
                                </p>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.total-refund')
                                </p>
                            ...
                            <div class="flex w-full justify-between gap-x-5 font-semibold">
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.total-due')
                                </p>
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <p class="p-4 pb-0 text-base font-semibold text-gray-800 dark:text-white">
                        @lang('superadmin::app.sales.orders.view.comments')
                    </p>

                    <x-superadmin::form action="{{ route('superadmin.sales.orders.comment', $order->id) }}">
                        <div class="p-4">
                            <div class="mb-2.5">
                                <x-superadmin::form.control-group>
                                    <x-superadmin::form.control-group.control
                                        ...
                                        name="comment"
                                        rules="required"
                                        :label="trans('superadmin::app.sales.orders.view.comments')"
                                        :placeholder="trans('superadmin::app.sales.orders.view.write-your-comment')"
                                        rows="3"
                                    />

                                    <x-superadmin::form.control-group.error control-name="comment" />
                                </x-superadmin::form.control-group>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                >
                                    ...
                                    <p class="flex cursor-pointer items-center gap-x-1 font-semibold text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-gray-100">
                                        @lang('superadmin::app.sales.orders.view.notify-customer')
                                    </p>
                                </label>
                                ...
                                <button
                                    ...
                                    aria-label="{{ trans('superadmin::app.sales.orders.view.submit-comment') }}"
                                >
                                    @lang('superadmin::app.sales.orders.view.submit-comment')
                                </button>
                            </div>
                        </div>
                    </x-superadmin::form>
                        ...
                        <div class="grid gap-1.5 p-4">
                            ...
                            <p class="flex items-center gap-2 text-gray-600 dark:text-gray-300">
                                @if ($comment->customer_notified)
                                    ...
                                    @lang('superadmin::app.sales.orders.view.customer-notified', ['date' => core()->formatDate($comment->created_at, 'Y-m-d H:i:s a')])
                                @else
                                    <span class="icon-cancel-1 h-fit rounded-full bg-red-100 text-2xl text-red-600"></span>

                                    @lang('superadmin::app.sales.orders.view.customer-not-notified', ['date' => core()->formatDate($comment->created_at, 'Y-m-d H:i:s a')])
                                @endif
                            </p>
                        </div>
            ...
            <div class="flex w-[360px] max-w-full flex-col gap-2 max-sm:w-full">
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <div class="flex items-center justify-between p-1.5">
                        <p class="p-2.5 text-base font-semibold text-gray-600 dark:text-gray-300">
                            @lang('superadmin::app.sales.orders.view.customer')
                        </p>
                    </div>
                    ...
                    <div class="px-4 pb-4" v-pre>
                        <div class="{{ $order->billing_address ? 'pb-4' : '' }}">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <p 
                                    ...
                                    v-pre
                                >
                                    @lang('superadmin::app.sales.orders.view.customer-group') : {{ $order->is_guest ? core()->getGuestCustomerGroup()?->name : ($order->customer->group->name ?? '') }}
                                </p>
                            ...
                            <div class="{{ $order->shipping_address ? 'pb-4' : '' }}">
                                ...
                                <div class="flex items-center justify-between">
                                    <p class="py-4 text-base font-semibold text-gray-600 dark:text-gray-300">
                                        @lang('superadmin::app.sales.orders.view.billing-address')
                                    </p>
                                </div>

                                @include ('superadmin::sales.address', ['address' => $order->billing_address])
                        ...
                        @if ($order->shipping_address)
                            ...
                            <div class="flex items-center justify-between">
                                <p class="py-4 text-base font-semibold text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.shipping-address')
                                </p>
                            </div>

                            @include ('superadmin::sales.address', ['address' => $order->shipping_address])
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <div class="flex items-center justify-between p-1.5">
                        <p class="p-2.5 text-base font-semibold text-gray-600 dark:text-gray-300">
                            @lang('superadmin::app.sales.orders.view.order-information')
                        </p>
                    </div>
                    ...
                    <div class="px-4 pb-4">
                        <div class="flex w-full justify-start gap-5">
                            <div class="flex flex-col gap-y-1.5">
                                <p class="text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.order-date')
                                </p>

                                <p class="text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.order-status')
                                </p>

                                <p class="text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.channel')
                                </p>
                            </div>
                            ...
                            <div class="flex flex-col gap-y-1.5">
                                ...
                                <!-- Order Status -->
                                <p class="text-gray-600 dark:text-gray-300">
                                    @if(($order->seller_approval_status ?? null) === 'pending')
                                        @lang('superadmin::app.sales.orders.index.datagrid.processing')
                                    @else
                                        {{ $order->status_label }}
                                    @endif
                                </p>
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <div class="flex items-center justify-between p-1.5">
                        <p class="p-2.5 text-base font-semibold text-gray-600 dark:text-gray-300">
                            @lang('superadmin::app.sales.orders.view.payment-and-shipping')
                        </p>
                    </div>
                    ...
                    <div class="px-4 pb-4">
                        <div>
                            ...
                            <p class="text-gray-600 dark:text-gray-300">
                                @lang('superadmin::app.sales.orders.view.payment-method')
                            </p>
                            ...
                            <p class="text-gray-600 dark:text-gray-300">
                                @lang('superadmin::app.sales.orders.view.currency')
                            </p>
                        ...
                        @if ($order->shipping_address)
                            ...
                            <div class="pt-4">
                                ...
                                <p class="text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.shipping-method')
                                </p>
                                ...
                                <p class="text-gray-600 dark:text-gray-300">
                                    @lang('superadmin::app.sales.orders.view.shipping-price')
                                </p>
                            </div>
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <div class="flex items-center justify-between p-1.5">
                        <p class="p-2.5 text-base font-semibold text-gray-600 dark:text-gray-300">
                            @lang('superadmin::app.sales.orders.view.invoices') ({{ count($order->invoices) }})
                        </p>
                    </div>
                    ...
                    <div class="px-4 pb-4">
                        @forelse ($order->invoices as $index => $invoice)
                            <div class="grid gap-y-2.5">
                                <div>
                                    <p class="font-semibold text-gray-800 dark:text-white">
                                        @lang('superadmin::app.sales.orders.view.invoice-id', ['invoice' => $invoice->increment_id ?? $invoice->id])
                                    </p>
                                ...
                                <div class="flex gap-2.5">
                                    <a
                                        href="{{ route('superadmin.sales.invoices.view', $invoice->id) }}"
                                        class="text-sm text-blue-600 transition-all hover:underline"
                                    >
                                        @lang('superadmin::app.sales.orders.view.view')
                                    </a>

                                    <a
                                        href="{{ route('superadmin.sales.invoices.print', $invoice->id) }}"
                                        class="text-sm text-blue-600 transition-all hover:underline"
                                    >
                                        @lang('superadmin::app.sales.orders.view.download-pdf')
                                    </a>
                                </div>
                            </div>
                        ...
                        @empty
                            <p class="text-gray-600 dark:text-gray-300">
                                @lang('superadmin::app.sales.orders.view.no-invoice-found')
                            </p>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <div class="flex items-center justify-between p-1.5">
                        <p class="p-2.5 text-base font-semibold text-gray-600 dark:text-gray-300">
                            @lang('superadmin::app.sales.orders.view.shipments') ({{ count($order->shipments) }})
                        </p>
                    </div>
                    ...
                    <div class="px-4 pb-4">
                        @forelse ($order->shipments as $shipment)
                            <div class="grid gap-y-2.5">
                                <div>
                                    <!-- Shipment Id -->
                                    <p class="font-semibold text-gray-800 dark:text-white">
                                        @lang('superadmin::app.sales.orders.view.shipment', ['shipment' => $shipment->id])
                                    </p>
                                ...
                                <div class="flex gap-2.5">
                                    <a
                                        href="{{ route('superadmin.sales.shipments.view', $shipment->id) }}"
                                        class="text-sm text-blue-600 transition-all hover:underline"
                                    >
                                        @lang('superadmin::app.sales.orders.view.view')
                                    </a>
                        ...
                        @empty
                            <p class="text-gray-600 dark:text-gray-300">
                                @lang('superadmin::app.sales.orders.view.no-shipment-found')
                            </p>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <div class="flex items-center justify-between p-1.5">
                        <p class="p-2.5 text-base font-semibold text-gray-600 dark:text-gray-300">
                            @lang('superadmin::app.sales.orders.view.refund')
                        </p>
                    </div>
                    ...
                    <div class="px-4 pb-4">
                        @forelse ($order->refunds as $refund)
                            <div class="grid gap-y-2.5">
                                <div>
                                    <p class="font-semibold text-gray-800 dark:text-white">
                                        @lang('superadmin::app.sales.orders.view.refund-id', ['refund' => $refund->id])
                                    </p>
                                    ...
                                    <p class="mt-4 font-semibold text-gray-800 dark:text-white">
                                        @lang('superadmin::app.sales.orders.view.name')
                                    </p>
                                    ...
                                    <p class="mt-4 font-semibold text-gray-800 dark:text-white">
                                        @lang('superadmin::app.sales.orders.view.status')
                                    </p>

                                    <p class="text-gray-600 dark:text-gray-300">
                                        @lang('superadmin::app.sales.orders.view.refunded')
                                ...
                                <div class="flex gap-2.5">
                                    <a
                                        href="{{ route('superadmin.sales.refunds.view', $refund->id) }}"
                                        class="text-sm text-blue-600 transition-all hover:underline"
                                    >
                                        @lang('superadmin::app.sales.orders.view.view')
                                    </a>
                        ...
                        @empty
                            <p class="text-gray-600 dark:text-gray-300">
                                @lang('superadmin::app.sales.orders.view.no-refund-found')
                            </p>
                        @endforelse
                    </div>
                </div>
