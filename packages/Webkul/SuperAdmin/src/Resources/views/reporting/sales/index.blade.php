<x-superadmin::layouts>
    <x-slot:title>
        @lang('superadmin::app.reporting.sales.index.title')
    </x-slot>
    ...
    <div class="flex flex-1 flex-col gap-4 max-xl:flex-auto">
        <!-- Sales Section -->
        @include('superadmin::reporting.sales.total-sales')
        ...
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            ...
            @include('superadmin::reporting.sales.purchase-funnel')

            <!-- Abandoned Carts Section -->
            @include('superadmin::reporting.sales.abandoned-carts')
        </div>

        <!-- Total Orders and Average Order Value Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Total Orders Section -->
            @include('superadmin::reporting.sales.total-orders')

            <!-- Average Order Value Section -->
            @include('superadmin::reporting.sales.average-order-value')
        </div>

        <!-- Tax Collected and Shipping Collected Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Tax Collected Section -->
            @include('superadmin::reporting.sales.tax-collected')

            <!-- Shipping Collected Section -->
            @include('superadmin::reporting.sales.shipping-collected')
        </div>

        <!-- Refunds and Top Payment Methods Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Refunds Section -->
            @include('superadmin::reporting.sales.total-refunds')

            <!-- Top Payment Methods Section -->
            @include('superadmin::reporting.sales.top-payment-methods')
        </div>
    </div>
            ...
            <div class="flex gap-1.5">
                ...
                <x-superadmin::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        ...
                        placeholder="@lang('superadmin::app.reporting.sales.index.start-date')"
                    />
                ...
                <x-superadmin::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        class="flex min-h-[39px] w-full rounded-md border px-3 py-2 text-sm text-gray-600 transition-all hover:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-gray-400"
                        v-model="filters.end"
                        placeholder="@lang('superadmin::app.reporting.sales.index.end-date')"
                    />
                </x-superadmin::flat-picker.date>
            </div>
        </script>
            ...
            window.app.component('v-reporting-filters', {
                ...
                data() {
                    return {
                        channels: [
                            {
                                name: "@lang('superadmin::app.reporting.sales.index.all-channels')",
                                code: ''
                            },
