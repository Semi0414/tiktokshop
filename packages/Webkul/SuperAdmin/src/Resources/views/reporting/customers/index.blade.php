<x-superadmin::layouts>
    <x-slot:title>
        @lang('superadmin::app.reporting.customers.index.title')
    </x-slot>
    ...
    <div class="flex flex-1 flex-col gap-4 max-xl:flex-auto">
        <!-- Customers Section -->
        @include('superadmin::reporting.customers.total-customers')
        ...
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            ...
            @include('superadmin::reporting.customers.most-sales')

            <!-- Customers With Most Orders Section -->
            @include('superadmin::reporting.customers.most-orders')
        </div>

        <!-- Customers Traffic Section -->
        @include('superadmin::reporting.customers.total-traffic')

        <!-- Top Customer Groups Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Top Customer Groups Section -->
            @include('superadmin::reporting.customers.top-customer-groups')

            <!-- Customer with Most Reviews Section -->
            @include('superadmin::reporting.customers.most-reviews')
        </div>
    </div>
            ...
            <div class="flex gap-1.5">
                ...
                <x-superadmin::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        ...
                        placeholder="@lang('superadmin::app.reporting.customers.index.start-date')"
                    />
                ...
                <x-superadmin::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        class="flex min-h-[39px] w-full rounded-md border px-3 py-2 text-sm text-gray-600 transition-all hover:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-gray-400"
                        v-model="filters.end"
                        placeholder="@lang('superadmin::app.reporting.customers.index.end-date')"
                    />
                </x-superadmin::flat-picker.date>
            </div>
        </script>
            ...
            window.app.component('v-reporting-filters', {
                ...
                data() {
                    return {
                        channels: [
                            {
                                name: "@lang('superadmin::app.reporting.customers.index.all-channels')",
                                code: ''
                            },
