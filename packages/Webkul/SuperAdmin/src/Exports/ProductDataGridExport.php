...
namespace Webkul\SuperAdmin\Exports;
...
class ProductDataGridExport implements FromCollection, ShouldAutoSize, WithHeadings, WithMapping
{
    ...
    public function headings(): array
    {
        ...
        return array_merge(
            ...
            $attributeHeaders,
            [
                trans('superadmin::app.catalog.products.edit.images.title'),
                trans('superadmin::app.catalog.products.edit.videos.title'),
            ]
        );
    }
    ...
    {
        // product_id → attribute_family_id (read directly from `products` so it
        // works for CSV-imported rows that aren't yet present in `product_flat`).
        $this->productFamilyMap = DB::table('products')
            ->whereIn('id', $productIds)
            ->pluck('attribute_family_id', 'id');
    ...
    protected function resolveAttributeValue(int $productId, object $attribute): mixed
    {
        ...
        return match ($attribute->type) {
            ...
            'boolean' => (bool) $value
                ? trans('superadmin::app.export.yes')
                : trans('superadmin::app.export.no'),
