...
class ExchangeRatesDataGrid extends DataGrid
{
    ...
    public function prepareColumns()
    {
        $this->addColumn([
            ...
            'label' => trans('superadmin::app.settings.exchange-rates.index.datagrid.id'),
            'type' => 'integer',
            'filterable' => true,
            'sortable' => true,
        ]);
        ...
        $this->addColumn([
            'index' => 'currency_name',
            'label' => trans('superadmin::app.settings.exchange-rates.index.datagrid.currency-name'),
            'type' => 'string',
            'searchable' => true,
            'filterable' => true,
            'sortable' => true,
        ...
        $this->addColumn([
            'index' => 'currency_rate',
            'label' => trans('superadmin::app.settings.exchange-rates.index.datagrid.exchange-rate'),
            'type' => 'integer',
            'searchable' => true,
            'filterable' => true,
            'sortable' => true,
    ...
    public function prepareActions()
    {
        if (bouncer()->hasPermission('settings.exchange_rates.edit')) {
            $this->addAction([
                'index' => 'edit',
                'icon' => 'icon-edit',
                'title' => trans('superadmin::app.settings.exchange-rates.index.datagrid.edit'),
                'method' => 'GET',
                'url' => function ($row) {
                    return route('superadmin.settings.exchange_rates.edit', $row->currency_exchange_id);
                },
            ]);
        }
        ...
        if (bouncer()->hasPermission('settings.exchange_rates.delete')) {
            $this->addAction([
                'index' => 'delete',
                'icon' => 'icon-delete',
                'title' => trans('superadmin::app.settings.exchange-rates.index.datagrid.delete'),
                'method' => 'DELETE',
                'url' => function ($row) {
                    return route('superadmin.settings.exchange_rates.delete', $row->currency_exchange_id);
                },
            ]);
        }
    }
