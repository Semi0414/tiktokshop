...
class CustomerDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        ...
        if (Schema::hasTable('seller') && Schema::hasColumn('customers', 'referral_seller_id')) {
            $queryBuilder->leftJoin('seller as referral_seller', 'customers.referral_seller_id', '=', 'referral_seller.id')
                ->addSelect(DB::raw('MAX('.$tablePrefix.'referral_seller.referral_code) as seller_referral_code'));
        } else {
            $queryBuilder->addSelect(DB::raw('NULL as seller_referral_code'));
        }
        ...
        if (Schema::hasColumn('customers', 'wallet_balance')) {
            $queryBuilder->addSelect(DB::raw('MAX('.$tablePrefix.'customers.wallet_balance) as wallet_balance'));
        } else {
            $queryBuilder->addSelect(DB::raw('0 as wallet_balance'));
        }

        if (Schema::hasColumn('customers', 'credit_score')) {
            $queryBuilder->addSelect(DB::raw('MAX('.$tablePrefix.'customers.credit_score) as credit_score'));
        } else {
            $queryBuilder->addSelect(DB::raw('0 as credit_score'));
        }
    ...
    public function prepareColumns()
    {
        ...
        $this->addColumn([
            'index' => 'seller_referral_code',
            'label' => trans('superadmin::app.customers.customers.index.datagrid.seller-referral-code'),
            'type' => 'string',
            'sortable' => false,
            'closure' => fn ($row) => $row->seller_referral_code ? e($row->seller_referral_code) : '—',
        ]);

        $this->addColumn([
            'index' => 'wallet_balance',
            'label' => trans('superadmin::app.customers.customers.index.datagrid.wallet-balance'),
            'type' => 'string',
            'sortable' => true,
        ...
        $this->addColumn([
            'index' => 'credit_score',
            'label' => trans('superadmin::app.customers.customers.index.datagrid.credit-score'),
            'type' => 'integer',
            'sortable' => true,
        ]);
    ...
    public function prepareActions()
    {
        $this->addAction([
            ...
            'method' => 'GET',
            'url' => function ($row) {
                return route('superadmin.customers.customers.view', $row->customer_id);
            },
        ]);

        $this->addAction([
            'icon' => 'icon-exit',
            'title' => trans('superadmin::app.customers.customers.index.datagrid.login-as-customer'),
            'method' => 'GET',
            'target' => 'blank',
            'url' => function ($row) {
                return route('superadmin.customers.customers.login_as_customer', $row->customer_id);
            },
        ]);
    }
    ...
    public function prepareMassActions()
    {
        if (bouncer()->hasPermission('customers.customers.delete')) {
            $this->addMassAction([
                ...
                'method' => 'POST',
                'url' => route('superadmin.customers.customers.mass_delete'),
            ]);
        }
        ...
        if (bouncer()->hasPermission('customers.customers.edit')) {
            $this->addMassAction([
                ...
                'method' => 'POST',
                'url' => route('superadmin.customers.customers.mass_update'),
                'options' => [
                    [
                        'label' => trans('superadmin::app.customers.customers.index.datagrid.active'),
                        'value' => 1,
