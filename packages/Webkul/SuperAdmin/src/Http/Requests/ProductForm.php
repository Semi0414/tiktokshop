...
namespace Webkul\SuperAdmin\Http\Requests;
...
use Webkul\SuperAdmin\Validations\ProductCategoryUniqueSlug;
...
class ProductForm extends FormRequest
{
            ...
            if ($attribute->is_unique) {
                array_push($validations, function ($field, $value, $fail) use ($attribute) {
                    if (
                        ...
                        $fail(trans('superadmin::app.catalog.products.index.already-taken', ['name' => ':attribute']));
                    }
                });
            }
    ...
    {
        return [
            'variants.*.sku.unique' => trans('superadmin::app.catalog.products.index.already-taken', ['name' => ':attribute']),
            'videos.files.*' => trans('superadmin::app.catalog.products.edit.videos.error', ['max' => $this->maxVideoFileSize]),
        ];
    }
