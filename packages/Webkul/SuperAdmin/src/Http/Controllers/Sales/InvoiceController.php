...
use Webkul\SuperAdmin\DataGrids\Sales\OrderInvoiceDataGrid;
...
class InvoiceController extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->superadminListing('superadmin::sales.invoices.index', [], OrderInvoiceDataGrid::class);
    }
    ...
    public function create(int $orderId)
    {
        ...
        return view('superadmin::sales.invoices.create', compact('order'));
    }
    ...
    public function store(int $orderId)
    {
        ...
        if (! $order->canInvoice()) {
            session()->flash('error', trans('superadmin::app.sales.invoices.create.creation-error'));
        ...
        if (! $this->invoiceRepository->haveProductToInvoice(request()->all())) {
            session()->flash('error', trans('superadmin::app.sales.invoices.create.product-error'));

            return redirect()->back();
        }

        if (! $this->invoiceRepository->isValidQuantity(request()->all())) {
            session()->flash('error', trans('superadmin::app.sales.invoices.create.invalid-qty'));
        ...
        session()->flash('success', trans('superadmin::app.sales.invoices.create.create-success'));

        return redirect()->route('superadmin.sales.orders.view', $orderId);
    }
    ...
    public function view(int $id)
    {
        ...
        return view('superadmin::sales.invoices.view', compact('invoice'));
    }
    ...
    public function sendDuplicateEmail(Request $request, int $id)
    {
        ...
        session()->flash('success', trans('superadmin::app.sales.invoices.view.invoice-sent'));

        return redirect()->route('superadmin.sales.invoices.view', $invoice->id);
    }
    ...
    public function massUpdateState(MassUpdateRequest $massUpdateRequest)
    {
        ...
        return new JsonResponse([
            'message' => trans('superadmin::app.sales.invoices.index.datagrid.mass-update-success'),
        ], 200);
    }
}
