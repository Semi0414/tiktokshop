...
use Webkul\SuperAdmin\DataGrids\Settings\UserDataGrid;
use Webkul\SuperAdmin\Http\Controllers\Controller;
use Webkul\SuperAdmin\Http\Requests\UserForm;
...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->superadminListing('superadmin::settings.users.index', compact('roles'), UserDataGrid::class);
    }
    ...
    public function store(UserForm $request): JsonResponse
    {
        ...
        if (request()->hasFile('image')) {
            $admin->image = current(request()->file('image'))->store('seller/'.$admin->id);
        ...
        return new JsonResponse([
            'message' => trans('superadmin::app.settings.users.create-success'),
        ]);
    }
    ...
    public function update(UserForm $request): JsonResponse
    {
        ...
        if ($data instanceof RedirectResponse) {
            return new JsonResponse([
                'message' => trans('superadmin::app.settings.users.update-success'),
            ]);
        }
    ...
    public function destroy($id): JsonResponse
    {
        if ($this->adminRepository->count() == 1) {
            return new JsonResponse([
                'message' => trans('superadmin::app.settings.users.last-delete-error'),
            ], 400);
        }

        if (auth()->guard('superadmin')->user()->id == $id) {
            return new JsonResponse([
                'message' => trans('superadmin::app.settings.users.delete-self-error'),
            ], 403);
        }
        ...
        try {
            ...
            return new JsonResponse([
                'message' => trans('superadmin::app.settings.users.delete-success'),
            ], 200);
        } catch (\Exception $e) {
        }

        return new JsonResponse([
            'message' => trans('superadmin::app.settings.users.delete-failed'),
        ], 500);
    }
    ...
    public function confirm($id)
    {
        ...
        return view('superadmin::customers.customers.confirm-password', compact('user'));
    }
    ...
    public function destroySelf(): JsonResponse
    {
        ...
        if (Hash::check($password, auth()->guard('superadmin')->user()->password)) {
            if ($this->adminRepository->count() == 1) {
                session()->flash('error', trans('superadmin::app.settings.users.delete-last'));
            } else {
                ...
                return new JsonResponse([
                    'redirectUrl' => route('superadmin.session.create'),
                    'message' => trans('superadmin::app.settings.users.delete-success'),
                ]);
            }
        } else {
            return new JsonResponse([
                'message' => trans('superadmin::app.settings.users.incorrect-password'),
            ], 404);
        }
    }
    ...
    private function prepareUserData(UserForm $request, $id)
    {
        ...
        if (
            ...
            && (auth()->guard('superadmin')->user()->id === (int) $id
    ...
    {
        session()->flash('error', trans('superadmin::app.settings.users.cannot-change', [
            'name' => $columnName,
        ]));

        return redirect()->route('superadmin.settings.users.index');
    }
}
