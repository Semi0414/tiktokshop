...
use Webkul\SuperAdmin\DataGrids\Settings\RolesDataGrid;
...
class RoleController extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->superadminListing('superadmin::settings.roles.index', [], RolesDataGrid::class);
    }
    ...
    public function store()
    {
        ...
        session()->flash('success', trans('superadmin::app.settings.roles.create-success'));

        return redirect()->route('superadmin.settings.roles.index');
    }
    ...
    public function update(int $id)
    {
        ...
        if (
            ...
            session()->flash('error', trans('superadmin::app.settings.roles.being-used'));
        ...
        session()->flash('success', trans('superadmin::app.settings.roles.update-success'));
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        if ($role->admins->count() >= 1) {
            return new JsonResponse(['message' => trans('superadmin::app.settings.roles.being-used', [
                'name' => 'superadmin::app.settings.roles.index.title',
                'source' => 'superadmin::app.settings.roles.index.admin-user',
            ])], 400);
        }
        ...
        if ($this->roleRepository->count() == 1) {
            return new JsonResponse([
                'message' => trans(
                    'superadmin::app.settings.roles.last-delete-error'
                ),
            ], 400);
        }
        ...
        try {
            ...
            return new JsonResponse(['message' => trans('superadmin::app.settings.roles.delete-success')]);
        } catch (\Exception $e) {
        }

        return new JsonResponse([
            'message' => trans(
                'superadmin::app.settings.roles.delete-failed'
            ),
        ], 500);
    }
}
