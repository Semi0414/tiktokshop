    ...
    @if ($sellerUser)
        <div class="mx-4 mb-4 shrink-0 rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-950">
            <div class="flex gap-3">
                <a
                    href="{{ route('admin.seller.visit-store') }}"
                    target="_blank"
                    ...
                    title="@lang('admin::app.seller-panel.sidebar.visit-store')"
                >
                    @if ($shopLogoUrl)
                        <img
                            src="{{ $shopLogoUrl }}"
                        ...
                        />
                    @else
                        <span aria-hidden="true">{{ mb_substr($sellerUser->name, 0, 1) }}</span>
                    @endif
                </a>
                ...
                <div class="min-w-0 flex-1">
                    ...
                    @if ($sellerLevelLabel)
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            @lang('admin::app.seller-panel.sidebar.seller-level-line', ['level' => $sellerLevelLabel])
                        </p>
                    @endif
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        @if (! empty($sellerUser->referral_code))
                            @lang('admin::app.seller-panel.sidebar.referral-code'):
                            <span class="font-mono font-semibold text-gray-700 dark:text-gray-200">{{ $sellerUser->referral_code }}</span>
                        @else
                            ID {{ $sellerUser->id }}
                        @endif
                    </p>
            ...
            <div class="mt-3 flex flex-col gap-2">
                ...
                >
                    @lang('admin::app.seller-panel.sidebar.visit-store')
                </a>
            </div>
        </div>
    @endif
            ...
            @foreach (menu()->getItems('admin') as $menuItem)
                ...
                >
                    <a
                        ...
                        class="flex items-center justify-between gap-2 p-1.5 cursor-pointer hover:rounded-lg {{ $menuItem->isActive() ? 'bg-blue-600 rounded-lg' : ' hover:bg-gray-100 hover:dark:bg-gray-950' }} peer"
                    >
                        <span class="flex min-w-0 items-center gap-2.5">
                            <span class="{{ $menuItem->getIcon() }} text-2xl shrink-0 {{ $menuItem->isActive() ? 'text-white' : ''}}"></span>
                        ...
                        @if ($menuItem->getKey() === 'sales' && $shopOrderBadgeCount > 0)
                            <span class="shrink-0 rounded-full bg-red-600 px-2 py-0.5 text-xs font-bold text-white group-[.sidebar-collapsed]/container:hidden">
                                {{ $shopOrderBadgeCount > 99 ? '99+' : $shopOrderBadgeCount }}
                            </span>
                        @endif
                    </a>
                    ...
                    @if ($menuItem->haveChildren())
                        ...
                        @foreach ($menuItem->getChildren() as $subMenuItem)
                            @if (! in_array($subMenuItem->getKey(), $allowedMenuKeys, true))
                                @continue
                            @endif
                                <a
