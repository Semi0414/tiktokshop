    ...
    <script type="module">
        app.component('v-datagrid', {
            ...
            mounted() {
                ...
                this._onSellerStoreGridRefresh = () => {
                    if (typeof this.src === 'string' && this.src.indexOf('store-products') !== -1) {
                        this.get();
                    }
                };

                window.emitter.on('seller-store-product-grid-refresh', this._onSellerStoreGridRefresh);
            },

            beforeUnmount() {
                if (this._onSellerStoreGridRefresh) {
                    window.emitter.off('seller-store-product-grid-refresh', this._onSellerStoreGridRefresh);
                }
            },
