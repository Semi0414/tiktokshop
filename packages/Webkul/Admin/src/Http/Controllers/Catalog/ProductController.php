...
class ProductController extends Controller
{
    ...
    /**
     * Ensure the authenticated seller may access the given catalog product.
     */
    protected function assertSellerCanAccessProduct(int $productId): void
    {
        $user = auth()->guard('admin')->user();

        if ($user instanceof Admin && ! $user->canAccessProductId($productId)) {
            abort(403);
        }
    }
    ...
    public function index()
    {
        ...
        $categoryFilterOptions = DB::table('category_translations')
            ->where('locale', app()->getLocale())
            ->orderBy('name')
            ->get(['category_id', 'name']);

        return view('admin::catalog.products.index', compact('families', 'categoryFilterOptions'));
    }
    ...
    public function edit(int $id)
    {
        ...
        $this->assertSellerCanAccessProduct((int) $product->id);

        $sellerStoreContext = null;

        if (bouncer()->hasPermission('product_management.catalog_products')) {
            /** @var Admin $seller */
            $seller = auth()->guard('admin')->user();
            $sellerStoreContext = [
                'sellerStoreProduct' => SellerStoreProduct::query()
        ...
        return view('admin::catalog.products.edit', compact('product', 'sellerStoreContext'));
    }
    ...
    {
        $this->assertSellerCanAccessProduct($id);
    ...
    public function massDestroy(MassDestroyRequest $massDestroyRequest): JsonResponse
    {
        ...
        foreach ($productIds as $productId) {
            $this->assertSellerCanAccessProduct((int) $productId);
        }
    ...
    public function search()
    {
        ...
        if ($user instanceof Admin && $user->sellerCatalogIsRestricted()) {
            $allowed = $user->resolveVisibleProductIds();

            if ($products instanceof AbstractPaginator) {
                $products->setCollection(
                    $products->getCollection()->filter(fn ($p) => in_array((int) $p->id, $allowed, true))
                );
            }
        }
