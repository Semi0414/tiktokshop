...
abstract class AbstractType
{
    ...
    public function prepareForCart($data)
    {
        ...
        if (! ShopCheckoutInventory::shouldSkipInventoryChecks() && ! $this->haveSufficientQuantity($data['quantity'])) {
    ...
    public function validateCartItem(CartItem $item): CartItemValidationResult
    {
        ...
        // Match prepareForCart: getFinalPrice() uses qty-1 tier (indexed min_price), not getFinalPrice(line qty),
        // so cart line totals stay consistent with the price set when the item was added.
        $basePrice = round($this->getFinalPrice(), 4);
