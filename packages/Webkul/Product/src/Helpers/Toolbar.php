...
class Toolbar
{
    ...
    public function getAvailableOrders(): Collection
    {
        return collect([
            ...
            [
                'title' => trans('product::app.sort-by.options.id-asc'),
                'value' => 'id-asc',
                'sort' => 'id',
                'order' => 'asc',
                'position' => 7,
    ...
    {
        $configured = core()->getConfigData('catalog.products.storefront.sort_by') ?? 'price-desc';

        $order = $this->getAvailableOrders()
            ->where('value', $configured)
            ->first();

        return $order ?? $this->getAvailableOrders()
            ->where('value', 'price-desc')
            ->firstOrFail();
    }
    ...
    public function getAvailableLimits(): Collection
    {
        if ($productsPerPage = core()->getConfigData('catalog.products.storefront.products_per_page')) {
            ...
            if (! in_array('20', $pages, true)) {
                array_unshift($pages, '20');
            }

            return collect($pages)->merge([50, 100])->unique()->sort()->values();
        }

        return collect([20, 12, 24, 36, 48, 50, 100]);
    }
