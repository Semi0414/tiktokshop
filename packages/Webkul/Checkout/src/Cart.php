...
class Cart
{
    ...
    public function updateItems(array $data): bool|\Exception
    {
        foreach ($data['qty'] as $itemId => $quantity) {
            ...
            if (! ShopCheckoutInventory::shouldSkipInventoryChecks() && ! $this->isItemHaveQuantity($item)) {
    ...
    public function getErrors()
    {
        ...
        if (! ShopCheckoutInventory::shouldSkipInventoryChecks() && ! $this->isItemsHaveSufficientQuantity()) {
            return [
        ...
        if (! ShopCheckoutInventory::shouldSkipInventoryChecks() && ! $this->haveMinimumOrderAmount()) {
