...
abstract class DataGrid
{
    ...
    /**
     * Run the same pipeline as {@see process()} but return the payload array (no HTTP response).
     * Used for Super Admin SSR datagrid pages without an AJAX listing request.
     */
    public function getFormattedData(): array
    {
        ...
        if ($this->isExportable()) {
            throw new \RuntimeException('Datagrid export must use process() with an HTTP response.');
        }
    ...
    /**
     * True when the filter payload has nothing that should constrain the query.
     */
    protected function isDatagridFilterValueEmpty(mixed $requestedValues): bool
    {
    ...
    protected function processRequestedFilters(array $requestedFilters)
    {
        ...
        foreach ($requestedFilters as $requestedColumn => $requestedValues) {
            if ($requestedColumn === 'all') {
                if (! is_array($requestedValues) || $this->isDatagridFilterValueEmpty($requestedValues)) {
                    continue;
                }
                ...
                if ($this->isDatagridFilterValueEmpty($requestedValues)) {
                    continue;
                }
