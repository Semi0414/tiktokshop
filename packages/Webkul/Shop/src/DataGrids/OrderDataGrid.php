...
class OrderDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        $queryBuilder = DB::table('orders')
            ->leftJoin('seller as order_seller', 'orders.seller_id', '=', 'order_seller.id')
            ->addSelect(
                'orders.id',
                'orders.increment_id',
                'orders.seller_id',
                ...
                'order_seller.name as seller_store_name'
            )
    ...
    public function prepareColumns()
    {
        $this->addColumn([
            ...
            'label' => trans('shop::app.customers.account.orders.seller-store'),
            'type' => 'string',
            'searchable' => true,
            'filterable' => true,
            'sortable' => true,
            'closure' => function ($row) {
                $name = $row->seller_store_name ? (string) $row->seller_store_name : '';
                $sid = isset($row->seller_id) && $row->seller_id !== null ? (int) $row->seller_id : null;
                if ($name === '' && ! $sid) {
                    return '—';
                }
                ...
                $parts = array_filter([
                    $name !== '' ? e($name) : null,
                    $sid ? '<span class="text-zinc-500">'.e(trans('shop::app.customers.account.orders.seller-id-label')).': '.$sid.'</span>' : null,
                ]);
