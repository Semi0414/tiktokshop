...
<head>
  ...
  <title>@if ($__sfLegacy)TikTok Shop @else {{ $__sfName }} — Marketplace @endif</title>
...
<nav>
  <div class="logo">
    ...
    <div class="logo-text">@if ($__sfLegacy)TikTok <span>Shop</span>@else {{ $__sfNameWords[0] ?? $__sfName }} @if (count($__sfNameWords) > 1)<span>{{ implode(' ', array_slice($__sfNameWords, 1)) }}</span>@endif @endif</div>
  </div>
  <div class="nav-right">
    <button class="lang-btn">🌐 English</button>
    <a class="nav-cta" href="{{ route('shop.landing.join') }}">Get Started</a>
  </div>
</nav>
...
<section class="hero">
  <div class="hero-content">
    <div class="hero-tag">@if ($__sfLegacy)TikTok Shop @else {{ $__sfName }} @endif</div>
    <h1 class="hero-title">@if ($__sfLegacy)platform's fully managed operation model @else Your marketplace for buyers and trusted sellers @endif</h1>
    <p class="hero-desc">
      @if ($__sfLegacy)
      ...
      @else
      Shop quality products, support independent merchants, and grow your business with tools for catalog, orders, and secure checkout — all on {{ $__sfName }}.
      @endif
    </p>
    <div class="hero-btns">
      @if ($__sfLegacy)
      <a class="btn-outline" href="{{ route('shop.customer.session.index') }}">Buyer</a>
      <a class="btn-outline" href="{{ route('admin.session.create') }}">Seller</a>
      <a class="btn-solid" href="{{ route('shop.landing.join') }}">Join</a>
      @else
      <a class="btn-outline" href="{{ route('shop.customer.session.index') }}">Shop as buyer</a>
      <a class="btn-solid" href="{{ route('shop.landing.join') }}">Become a seller</a>
      <a class="btn-outline" href="{{ route('admin.session.create') }}" title="Seller Dashboard">Seller Dashboard</a>
      @endif
    </div>
    <div class="hero-social-proof">
      ...
      <span>@if ($__sfLegacy)50,000+ Users download our app @else Independent sellers and buyers on {{ $__sfName }} @endif</span>
    </div>
  </div>

  <div class="hero-visual">
    <div class="hero-media @if (!isset($landingHeroMedia) || $landingHeroMedia->isEmpty()) hero-media--empty @endif">
      @if (isset($landingHeroMedia) && $landingHeroMedia->isNotEmpty())
        <div class="ttk-lp-hero-slideshow" id="ttk-lp-hero-slideshow" aria-hidden="true">
          @foreach ($landingHeroMedia as $m)
            <div class="ttk-lp-hero-slide @if ($loop->first) ttk-lp-hero-slide--active @endif">
              @if ($m->isVideo())
                @php
                ...
                <video muted loop playsinline preload="auto">
                  <source src="{{ $m->getPublicUrl() }}" type="{{ $hpmVmime }}">
                </video>
              @else
                <img src="{{ $m->getPublicUrl() }}" alt="" decoding="async" @if ($loop->first) loading="eager" fetchpriority="high" @else loading="lazy" @endif />
              @endif
            </div>
          @endforeach
        </div>
...
<section class="features">
  <div class="features-grid">
    ...
    <div class="feature-card">
      ...
      <p>@if ($__sfLegacy)Creators shop up differently on TikTok, bringing a fresh take on shopping by helping brands build trust and influence through authentic content.@else Creators and brands connect with shoppers through authentic content, building trust and repeat purchases.@endif</p>
    </div>
    <div class="feature-card">
      <div class="feature-icon icon-community">👥</div>
      <h3>Engaging Communities</h3>
      <p>@if ($__sfLegacy)TikTok communities drive discovery and influence culture. Brands can build lasting relationships with users by tapping into these existing communities.@else Community and discovery help merchants grow relationships with customers who care about their niche.@endif</p>
    </div>
    <div class="feature-card">
      <div class="feature-icon icon-safe">🛒</div>
      <h3>Easy &amp; Safe Shopping</h3>
      <p>@if ($__sfLegacy)Revolutionising the shopping journey, from the For You Page to a safe and secure check-out, order management, and support all within the app.@else From product discovery to checkout, order management, and support — a straightforward, secure shopping experience.@endif</p>
    </div>
  </div>
</section>
...
<section class="global">
  <div class="global-text">
    ...
    <div class="hero-btns">
      <a class="btn-solid" href="{{ route('shop.landing.join') }}">Start Selling</a>
      <!-- <a class="btn-outline" href="{{ route('shop.customer.session.index') }}">Learn More</a> -->
    </div>
    <div class="stats-row">
      @if ($__sfLegacy)
      <div class="stat">
  ...
  <div class="global-visual">
    ...
    <div class="global-card">
      ...
      <div class="global-card-body">
        ...
        <p>@if ($__sfLegacy)From Southeast Asia to Europe and Americas — TikTok Shop connects you to customers everywhere. Streamlined shipping, payments, and support.@else Reach customers where they are — list products, manage orders, and offer support through {{ $__sfName }}. Shipping and payment options depend on your configuration and sellers.@endif</p>
      </div>
    </div>
  </div>
</section>
...
<section class="sellers-cta">
  ...
  <p>@if ($__sfLegacy)Join millions of sellers who have grown their business on TikTok Shop. Set up your store in minutes and start reaching customers today.@else Apply to sell on {{ $__sfName }}, complete verification, and list your catalog. Buyers discover your store through our marketplace.@endif</p>
  <div class="cta-btns">
    <a class="btn-big-red" href="{{ route('shop.landing.join') }}">Start as Seller</a>
    <a class="btn-big-outline" href="{{ route(\Webkul\Shop\Support\ShopRoutes::browseRouteName()) }}">Browse as Buyer</a>
  </div>
</section>
...
<footer>
  <div class="footer-top">
    <div class="footer-brand">
      <div class="logo">
        <div class="logo-text" style="font-size:1.3rem;">@if ($__sfLegacy)TikTok <span style="color:var(--red)">Shop</span>@else {{ $__sfNameWords[0] ?? $__sfName }} @if (count($__sfNameWords) > 1)<span style="color:var(--red)">{{ implode(' ', array_slice($__sfNameWords, 1)) }}</span>@endif @endif</div>
      </div>
      <p>@if ($__sfLegacy)The platform's fully managed operation model — providing quality goods and services to global consumers.@else {{ $__sfName }} provides marketplace software and seller onboarding. Products and fulfillment are offered by individual sellers, not by TikTok or ByteDance.@endif</p>
    </div>
  ...
  <div class="footer-bottom">
    <p>© {{ date('Y') }} @if ($__sfLegacy)TikTok Shop @else {{ config('app.name') }} @endif. All rights reserved.</p>
  ...
  <p class="footer-powered">
    <a href="{{ url('/') }}" rel="home">Powered by TikTok Shop, an open-source project by TikTok.</a>
  </p>
</footer>

@include('shop::components.layouts.storefront-chat-widgets')

@isset($landingHeroMedia)
  @if ($landingHeroMedia->isNotEmpty())
    <script>
      (function () {
