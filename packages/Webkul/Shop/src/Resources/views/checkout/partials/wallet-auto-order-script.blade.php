{{-- Testing alerts disabled (were enabled via APP_ENV=local or ?cart_debug=1) --}}
<script>
window.shopWalletCheckout = (function () {
    var loginUrl = @json(route('shop.customer.session.index'));
    var cartStoreUrl = @json(route('shop.api.checkout.cart.store'));
    var orderStoreUrl = @json(route('shop.checkout.onepage.orders.store'));
