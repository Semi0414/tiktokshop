...
class CartController extends APIController
{
    ...
    public function store()
    {
        if (! auth()->guard('customer')->check()) {
            return response()->json([
                'message' => trans('shop::app.customer.session.login-required'),
        ...
        if ($pid > 0 && $bodyProductId > 0 && $pid !== $bodyProductId) {
            return response()->json([
                'message' => trans('shop::app.checkout.cart.product-id-mismatch'),
        ...
        if ($effectiveProductId < 1) {
            return response()->json([
                'message' => trans('validation.required', ['attribute' => 'product_id']),
            ], Response::HTTP_UNPROCESSABLE_ENTITY);
        }
    ...
    /**
     * Bump sellable qty on the default inventory source so higher quantities (e.g. 2–3+) do not fail on add-to-cart.
     */
